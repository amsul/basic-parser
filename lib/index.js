...
const render: RenderType = ({ contexts, data, text }) => {
  ...
  while (contexts.length) {
    ...
    if (
      ...
      !currentContext.renderer.render
    ) {
      console.error(
        'No renderer found for the matching context in %o at %o',
        contexts,
        currentIndex,
      )
      break
    ...
    const hasContextCloser = contexts.some(
      (context: any) =>
    ...
    const nextContextIndex = contexts.findIndex(
      (context: any, index) =>
        index > currentIndex &&
        context &&
        context.renderer &&
    ...
    if (currentIndex !== 0) {
      contexts.splice(
        currentIndex,
        nextContextIndex - currentIndex + 1,
        element,
      )
    } else {
...
const normalizeRenderers = ({ renderers }): NormalizedRendererType[] => {
  return (renderers.reduce((accumulator, renderer, index) => {
    const previousMatchIndexes: NormalizedRendererType = (accumulator[
      accumulator.length - 1
    ]: any)
    const previousLastGroupIndex = previousMatchIndexes
      ? previousMatchIndexes.groupIndexes[
          previousMatchIndexes.groupIndexes.length - 1
        ]
      : 0
    ...
    if (typeof match === 'string') {
      ...
      accumulator.push({
        ...renderer,
        regExpPart: match,
        groupIndexes: Array.from(new Array(groupsCount)).map(
          (_, index) => index + previousLastGroupIndex + 1,
        ),
      })
    } else {
      const startGroupsCount =
        getGroupsCount({ regExp: new RegExp(match.start) }) + 1
      accumulator.push({
        ...renderer,
        regExpPart: match.start,
        groupIndexes: Array.from(new Array(startGroupsCount)).map(
          (_, index) => index + previousLastGroupIndex + 1,
        ),
      })
      const endGroupsCount =
        getGroupsCount({ regExp: new RegExp(match.end) }) + 1
      accumulator.push({
        ...renderer,
        regExpPart: match.end,
        groupIndexes: Array.from(new Array(endGroupsCount)).map(
          (_, index) => index + previousLastGroupIndex + 1 + startGroupsCount,
        ),
      })
    }
